...
import { CreateEmailComponent } from './create-email-component/create-email-component';
import { TblAction, TblActionTypes } from '../sms/tbl-action/tbl-action';
...
export class Email extends BaseComponent {
    ...
    onOpenCreate() {
        const ref = this._dialogService.open(CreateEmailComponent, { header: 'Tạo chiến dịch', closable: true, modal: true, styleClass: 'w-[600px]', focusOnShow: false });
    ...
    onOpenUpdate(data: IViewRowChienDich) {
        const ref = this._dialogService.open(CreateEmailComponent, { header: 'Tạo chiến dịch', closable: true, modal: true, styleClass: 'w-[600px]', focusOnShow: false, data });
        ref.onClose.subscribe((result) => {
            if (result) {
                this.getData();
            }
    ...
    navigateToDetail(chienDich: IViewRowChienDich) {
        if (chienDich?.id) {
            this.router.navigate(['/channel/gui-email'], {
                queryParams: {
